...
class UpdateInput(BaseModel):
    ...
    commit_message: str = Field(default="Working docs update via Claude", max_length=200)
...
@mcp.tool(name="workdocs_search", annotations={"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True})
async def workdocs_search(params: SearchInput) -> str:
    """Search working documents — PRDs, journals, changelogs, research, business docs, content plans, Oculus Anubis investigation."""
    ...
    if not results: return json.dumps({"query": params.query, "results": [], "message": "No matches. Try broader terms or workdocs_list."}, indent=2)
    return json.dumps({"query": params.query, "total_matches": len(all_matches), "showing": len(results), "results": results}, indent=2)

@mcp.tool(name="workdocs_read", annotations={"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True})
async def workdocs_read(params: ReadInput) -> str:
    """Read a working document by path."""
...
@mcp.tool(name="workdocs_update", annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": True})
async def workdocs_update(params: UpdateInput) -> str:
    """Create or update a working document. Auto-committed to git."""
    filepath = _safe_path(params.path)
...
@mcp.tool(name="workdocs_list", annotations={"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True})
async def workdocs_list() -> str:
    """List all working documents with paths, sizes, and last-modified dates."""
