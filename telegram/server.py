"""Telegram MCP Server — Telegram Bot API wrapper.
Tools: send_message, send_document, get_updates, get_chat_info, get_me, pin_message, set_chat_description
"""
import os
import httpx
...
async def send_message(text: str, chat_id: str = "", parse_mode: str = "HTML") -> str:
    ...
    if not cid: return "Error: no chat_id provided and TELEGRAM_CHAT_ID not set"
    result = await _api("sendMessage", chat_id=cid, text=text, parse_mode=parse_mode)
    if result.get("ok"): msg = result["result"]; return f"Sent message {msg['message_id']} to chat {cid}"
...
async def send_document(chat_id: str = "", document_url: str = "", caption: str = "") -> str:
    ...
    cid = chat_id or DEFAULT_CHAT_ID
    if not cid: return "Error: no chat_id"
    params = {"chat_id": cid, "document": document_url}
    if caption: params.update({"caption": caption, "parse_mode": "HTML"})
...
async def get_updates(limit: int = 10, offset: int = 0) -> str:
    ...
    if not result.get("ok"): return f"Error: {result}"
    updates = result.get("result", [])
    if not updates: return "No new updates"
    lines = []
    for u in updates:
        msg = u.get("message", {}); fr = msg.get("from", {})
        lines.append(f"[{u['update_id']}] {fr.get('first_name', '?')}: {msg.get('text', '(no text)')}")
    return "\n".join(lines)
...
async def get_me() -> str:
    ...
    if result.get("ok"): b = result["result"]; return f"Bot: {b['first_name']} (@{b.get('username', '?')}), ID: {b['id']}"
    return f"Error: {result}"
